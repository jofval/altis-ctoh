...
class ColinAnal_Panel(wx.Frame):
    """
        
    """

    def __init__(self, parent, data_opt, param_name):
        super().__init__(
            parent,
            title="AlTiS : Colinear Analysis",
    ...
    def CanvasPanel(self, panel):
        ...
        self.ax2.set_ylabel("Sample number")
        ...
        self.ax4.set_ylabel("Sample number")
        self.ax4.autoscale()
        ...
        multi = MultiCursor(self.canvas, (self.ax1, self.ax3), color="y", lw=1)
    ...
    def onCSVexport(self, event):
        ...
        for param in list_param_coord:
            array.append(
                self.common_data.tr[param].where(mask).mean(dim=self.dim_index).data
            )
            header_csvfile.extend([param + "_mean"])

        for param in list_param:
            array.append(
                self.common_data.tr[param].where(mask).median(dim=self.dim_index).data
            )
            array.append(
                self.common_data.tr[param].where(mask).mean(dim=self.dim_index).data
            )
            array.append(
                self.common_data.tr[param].where(mask).std(dim=self.dim_index).data
            )
        ...
        default_filaname = (
            ...
            + "_{:04d}".format(self.data_sel_config["track"])
        )

        latpos = (
            self.common_data.lat.where(mask).mean(dim=self.dim_index).mean(dim="cycle")
        )
        if latpos >= 0.0:
    ...
    def onpick(self, event):
        ...
        for subplotnum, dataind in enumerate(event.ind):
            ...
            ax.set_title(
                "Cycle number : %04d,\nMean value = %1.3f, Std = %1.3f "
                % (dataind, mean_H_param[dataind], std_H_param[dataind])
            )
